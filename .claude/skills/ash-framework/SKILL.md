---
name: ash-framework
description: "Use this skill when working with Ash Framework or any of its extensions (resources, domains, actions, policies, multitenancy, AshAuthentication, AshPostgres, AshOban). Always consult it before making domain changes."
metadata:
...
```sh
mix usage_rules.search_docs "search term" -p ash -p ash_authentication -p ash_authentication_phoenix -p ash_oban -p ash_phoenix -p ash_postgres
```
